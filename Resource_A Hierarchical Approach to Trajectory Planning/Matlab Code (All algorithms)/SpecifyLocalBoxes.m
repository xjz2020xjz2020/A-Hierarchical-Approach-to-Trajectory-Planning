...
for ii = 1 : NE
    ...
    cos_theta_i=cos_theta(ii) ;
    sin_theta_i=sin_theta(ii) ; 
    
    lb = GetBoxVertexes(x, y, theta_i, ii);   %  规划的轨迹 无 碰撞
    if (~all(lb))                             %  规划的轨迹只要一侧发生碰撞，必须往一边偏移，
        counter = 0;
        is_lb_nonzero = 0;
        while (~is_lb_nonzero)
            counter = counter + 1;
...
global vehicle_geometrics_
half_width = (vehicle_geometrics_.vehicle_width + vehicle_geometrics_.vehicle_residual) * 0.5;
...
lb = [ half_width, rear_length ,half_width, front_length ];  % 上，后，下，前
