function [v, a, phy, w, k, r, min_r, ind, r_temp] = FormInitialGuess(x, y, theta, tf, lim_r)  
% 返回 速度v，加速度a，前轮转角phy，前轮的转向角速度w, 曲率k，曲率半径r，最小半径min_r，小于lim_r的节点号ind，小于lim_r的半径r_temp
Nfe = length(x);
v = zeros(1, Nfe);
a = zeros(1, Nfe);
dt = tf / (Nfe - 1);
...
[k, r, min_r, ind, r_temp] = curvatute_r ( x, y, delta_t, lim_r );

end

function [k, r, min_r, ind, r_temp ] = curvatute_r ( x, y, t,lim_r )
   Nfe = length(x);
   for ii = 2 : Nfe
         dx(ii) = ( x(ii) - x(ii-1) )/t ;
   end
